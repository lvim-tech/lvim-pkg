...
Neovim's built-in `vim.treesitter` uses them. Each catalogue is fetched once on the
first setup (when its on-disk cache is missing) and refreshed only on demand.
...
periodic/TTL refresh. Force a re-download on demand via
`:LvimInstaller update-registry [mason|ts|all]`.
